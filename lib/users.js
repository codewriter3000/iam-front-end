...
export async function forcePasswordReset(userID) {
    const response = await apiFetch(`/user/${userID}/password/force-reset`, {
        method: 'POST',
    });
    ...
    if (!response.ok) {
        throw new Error(body?.error || 'Failed to force password reset');
    }
...
export async function getForcedResetContext() {
    const response = await apiFetch('/auth/password/reset/context', { method: 'GET' });
    const body = await response.json();

    if (!response.ok) {
        throw new Error(body?.error || 'Forced reset context unavailable');
    }
